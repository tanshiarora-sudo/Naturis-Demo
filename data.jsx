...
const NOTIFICATIONS = [
  { id: "N-101", type: "dispatch", severity: "info", title: "Dispatch awaiting your approval — SPF50 Mineral Fluid", body: "Lab prepared the dispatch note + 2 photos (docket DTDC-885214). Approve to send to Nykaa.", at: "9 Jun, 14:05", read: false, req: "REQ-2026-0351", rule: "NR-04", to: ["spoc"] },
  { id: "N-102", type: "flag", severity: "med", title: "Lab query on Saffron Lux Eye Concentrate", body: "Saffron 0.5% tints the gel amber — confirm acceptable colour range with Nua.", at: "8 Jun, 16:40", read: false, req: "REQ-2026-0407", rule: "NR-05", to: ["spoc"] },
  { id: "N-103", type: "flag", severity: "high", title: "Lab declined Ceramide Barrier Cream", body: "₹38/unit unviable for 1% ceramide — revise budget or descope. Back with Sales.", at: "8 Jun, 11:20", read: false, req: "REQ-2026-0412", rule: "NR-05", to: ["spoc"] },
  { id: "N-104", type: "vvip", severity: "info", title: "VVIP requirement logged — Vitamin-C Brightening Serum", body: "Nykaa NPD, EU Q4 launch. Awaiting Sales Manager review.", at: "8 Jun, 10:15", read: false, req: "REQ-2026-0431", rule: "NR-02", to: ["manager"] },
  { id: "N-105", type: "flag", severity: "med", title: "Manager returned Redensyl Hair Growth Serum", body: "Need confirmed FG target + MOQ from Asaya before NPD bandwidth.", at: "7 Jun, 17:05", read: false, req: "REQ-2026-0429", rule: "NR-05", to: ["spoc"] },
  { id: "N-106", type: "sla", severity: "high", title: "SLA breach — Coconut Milk Body Lotion", body: "Client unresponsive 9 days post-dispatch; lab phase 4d over budget.", at: "7 Jun, 09:30", read: true, req: "REQ-2026-0340", rule: "NR-03", to: ["spoc","manager"] },
  { id: "N-107", type: "flag", severity: "high", title: "Material short on Keratin Repair Hair Mask — solution issued", body: "Kunal approved alternate supplier (Aarna Chem, 3-day lead). Slot moved to 14 Jun.", at: "6 Jun, 12:45", read: true, req: "REQ-2026-0375", rule: "NR-05", to: ["lab","planner","labmgr"] },
  { id: "N-108", type: "sla", severity: "info", title: "Stability update — Vitamin C Glow Serum", body: "Month 2/6 complete — all parameters ok. Next check 12 Jul.", at: "5 Jun, 08:00", read: true, req: "REQ-2026-0327", rule: "NR-03", to: ["lab","spoc"] },
  { id: "N-109", type: "dispatch", severity: "info", title: "Lab committed 25 Jun on Oud Renewal Night Cream", body: "VVIP · accepted with tentative sample date 25 Jun (jar lead 3d).", at: "4 Jun, 15:30", read: true, req: "REQ-2026-0401", rule: "NR-04", to: ["planner","spoc"] },
  { id: "N-110", type: "dispatch", severity: "info", title: "Client approved Kumkumadi Face Oil", body: "Nua approved the sample. Stability decision pending (TT — prompt).", at: "4 Jun, 11:10", read: true, req: "REQ-2026-0394", rule: "NR-04", to: ["lab","spoc"] },
];
...
window.NaturisStore = {
  ...
  markAllRead(role) { (role ? this.notifsFor(role) : NOTIFICATIONS).forEach(n => { n.read = true; }); _bump(); },
  ...
  setLabStage(id, stage, by) { const r = this.get(id); if (r) { r.labStage = stage; r.labStageLog = r.labStageLog || {}; r.labStageLog[stage] = { by: by, at: new Date().toLocaleDateString("en-GB", { day: "numeric", month: "short" }) }; const core = window.NaturisData.LAB_STAGE_TO_STATUS[stage]; if (core && ["Accepted — date committed", "Formulation", "Trial", "QC", "Fill", "Ready for dispatch"].includes(r.status)) r.status = core; this.log(id, { kind: "status", icon: "work", stage: stage, actor: by, role: "Lab", at: "just now", detail: "Live lab status → " + stage, current: true }); this._notify(id, "dispatch", "info", id + " — " + stage, "Live lab status updated by the lab.", "NR-04", ["spoc"]); } _bump(); },
  setLabStageNote(id, stage, note, by) { const r = this.get(id); if (r) { r.labStageLog = r.labStageLog || {}; r.labStageLog[stage] = Object.assign({ by: by, at: new Date().toLocaleDateString("en-GB", { day: "numeric", month: "short" }) }, r.labStageLog[stage], { note: note }); } _bump(); },
  setPrePO(id, key, val, by) { const r = this.get(id); if (r) { r.prePO = r.prePO || {}; r.prePO[key] = val; const item = (window.NaturisData.PRE_PO_ITEMS.find(x => x[0] === key) || [])[1] || key; this.log(id, { kind: val ? "approval" : "status", icon: val ? "check" : "work", stage: "Pre-PO checklist", actor: by, role: "System", at: "just now", detail: item + (val ? " ✓ confirmed" : " un-ticked") }); const all = window.NaturisData.PRE_PO_ITEMS.every(x => r.prePO[x[0]]); if (all && !r.prePOComplete) { r.prePOComplete = true; this.log(id, { kind: "approval", icon: "star", stage: "Customer-ready", actor: "System", role: "System", at: "just now", detail: "Pre-PO checklist complete — customer-ready, awaiting PO.", current: true }); this._notify(id, "dispatch", "info", id + " is customer-ready — PO awaited", "All pre-PO checks done. Follow up for the PO.", "NR-04", ["spoc"]); } if (!all) r.prePOComplete = false; } _bump(); },
  // VVIP is an ACCOUNT-level attribute — promoting one promotes the whole brand and all its requirements
  setVvipOverride(id, val, by) { const r = this.get(id); if (r) { const acctName = r.account || r.brand; const acct = (window.NaturisData.ACCOUNTS || []).find(a => a.name === acctName || a.name === r.brand); this.addAudit({ actor: by, role: "Management", action: "VVIP override", target: acctName, field: "vvip", before: String(acct ? acct.vvip : r.vvip), after: String(val), note: "Account-level VVIP set from command centre" }); if (acct) acct.vvip = val; REQUIREMENTS.forEach(x => { if ((x.account || x.brand) === acctName || x.brand === r.brand) x.vvip = val; }); this.log(id, { kind: "status", icon: "star", stage: "VVIP " + (val ? "set" : "removed"), actor: by, role: "Management", at: "just now", detail: acctName + " " + (val ? "marked VVIP" : "VVIP removed") + " — applies to the whole brand & all its requirements." }); } _bump(); },
  // SPOC relays the client's feedback → same requirement, NEW formulation code, back to the lab to re-evaluate
  clientFeedback(id, text, by) { const r = this.get(id); if (r) { const seq = (r.id.match(/\d{3,}$/) || ["001"])[0].slice(-3); const iter = (r.ncls.length + 1).toString().padStart(3, "0"); const code = `NCL-${seq}-${iter}`; r.ncls.forEach(n => n.status = "superseded"); r.ncls.push({ code, by, at: "just now", delta: "Client feedback: " + text, status: "current" }); r.currentNcl = code; r.iteration = (r.iteration || 1) + 1; r.status = "In evaluation"; r.owner = "Lab"; r.clientFeedback = text; if (r.evaluation && r.evaluation.booking) { r.evaluation.booking = null; r.evaluation.slot = ""; } this.log(id, { kind: "flag", icon: "history", stage: "Client feedback", actor: by, role: "SPOC", at: "just now", detail: "Client feedback relayed → " + code + ": " + text + " (back to lab for re-evaluation).", current: true }); this._notify(id, "queue", "med", id + " — client feedback, re-evaluate", code + " · " + text, "NR-05", ["lab", "labmgr"]); } _bump(); },
  ...
  // ---- lab lifecycle ----
  acknowledge(id, by) { const r = this.get(id); if (r) { r.status = "Acknowledged"; this.log(id, { kind: "handoff", icon: "incoming", stage: "Acknowledged", actor: by, role: "Lab", at: "just now", detail: "Lab acknowledged (seen & reviewed) — moved to Under evaluation for the doability review.", current: true }); this._notify(id, "queue", "info", "Lab acknowledged " + id, "Reviewed by " + by + " — now under evaluation.", "NR-04", ["spoc"]); this._notify(id, "queue", "info", id + " is under evaluation", "Review doability at the lab meeting; assign a chemist if needed.", "NR-04", ["labmgr"]); } _bump(); },
  assignChemist(id, chemist, by) { const r = this.get(id); if (r) { r.tracker = chemist; r.assigned = true; if (r.status !== "Acknowledged" && !["In evaluation"].includes(r.status)) r.status = "Acknowledged"; this.log(id, { kind: "assigned", icon: "team", stage: "Chemist assigned", actor: by, role: "Lab Manager", at: "just now", detail: "Assigned to " + chemist + " for evaluation.", current: true }); this._notify(id, "queue", "info", id + " assigned to " + chemist, "Lab manager assigned this query to you.", "NR-04", ["lab"]); } _bump(); },
  ...
  newIteration(id, delta, by) { const r = this.get(id); if (r && !["Archived", "In stability"].includes(r.status)) { const seq = (r.id.match(/\d{3,}$/) || ["001"])[0].slice(-3); const iter = (r.ncls.length + 1).toString().padStart(3, "0"); const code = `NCL-${seq}-${iter}`; r.ncls.forEach(n => n.status = "superseded"); r.ncls.push({ code, by, at: "just now", delta: delta || "New iteration", status: "current" }); r.currentNcl = code; r.iteration = (r.iteration || 1) + 1; r.status = "Formulation"; r.labStage = null;
    // a reworked batch needs a fresh station slot — release the old booking and send it back to the planning queue
    if (r.evaluation && r.evaluation.booking) { r.evaluation.booking = null; r.evaluation.slot = ""; this._notify(id, "queue", "info", id + " back on the bench — needs a new slot", "Iteration " + code + " — re-schedule on the planning desk.", "NR-04", ["planner"]); }
    this.log(id, { kind: "status", icon: "history", stage: "Iteration " + r.iteration, actor: by, role: "Lab", at: "just now", detail: "New iteration " + code + (delta ? " — " + delta : "") + " (back to bench)", current: true }); } _bump(); },
  // ---- ownership + notify helpers ----
  _notify(id, type, sev, title, body, rule, to) { NOTIFICATIONS.unshift({ id: "N-" + (++_evSeq), type, severity: sev, title, body, at: "just now", read: false, req: id, rule: rule || "NR-04", to: to || ["all"] }); },
  // role-routed feed: a notification reaches a role if addressed to it (or "all"); mgmt & admin see everything (oversight)
  notifsFor(role) { return NOTIFICATIONS.filter(function (n) { var to = n.to || ["all"]; if (role === "admin" || role === "mgmt") return true; return to.indexOf("all") >= 0 || to.indexOf(role) >= 0; }); },
  ...
  clientOutcome(id, outcome, by) { const r = this.get(id); if (r) { if (outcome === "approved") { r.status = "Client approved"; this.log(id, { kind: "approval", icon: "check", stage: "Client approved", actor: by, role: "SPOC", at: "just now", detail: "Client approved the sample.", current: true }); this._notify(id, "dispatch", "info", id + " — client approved", "Start stability and prepare the ingredient sheet + marketing brief.", "NR-04", ["lab", "labmgr"]); } else if (outcome === "rejected") { r.status = "Rejected"; this.log(id, { kind: "flag", icon: "x", stage: "Rejected", actor: by, role: "SPOC", at: "just now", severity: "high", detail: "Client rejected.", current: true }); this._notify(id, "flag", "high", id + " — client rejected", "Client rejected the sample.", "NR-05", ["lab", "labmgr"]); } } _bump(); },
  // ---- stability ----
  startStability(id, months, by) { const r = this.get(id); if (r) { r.stability = { months, month: 0, status: "running", startedAt: "just now", m: ["", "", "", "", "", ""] }; r.status = "In stability"; this.log(id, { kind: "status", icon: "clock", stage: "Stability started", actor: by, role: "Lab", at: "just now", detail: `${months}-month stability cycle initiated.`, current: true }); } _bump(); },
  // log the actual date a monthly stability pull was checked (lab tech / management) — replaces tick marks
  setStabilityMonth(id, k, date) { const r = this.get(id); if (r) { r.stability = r.stability || { months: 6, month: 0, status: "running", m: ["", "", "", "", "", ""] }; r.stability.m = r.stability.m || ["", "", "", "", "", ""]; r.stability.m[k] = date; if (date) r.stability.month = Math.max(r.stability.month || 0, k + 1); } _bump(); },
